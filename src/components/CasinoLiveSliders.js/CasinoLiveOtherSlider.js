...
function CasinoLiveOtherSlider(props) {
	// const { casinoData } = useOutletContext();

	const [swiperRef, setSwiperRef] = useState(null);
	const navigationPrevRefLive4 = React.useRef(null);
	const navigationNextRefLive4 = React.useRef(null);
	const navigate = useNavigate();
	const handleClick = () => {
		// 👇️ replace set to true
		// navigate("casino-live/popular", { replace: true });
		window.location.replace("/casino-live/other");
	};
	return (
		<div>
			<div className=' relative bg-purple-800 w-full overflow-x-hidden py-5 px-5 space-y-5'>
				...
				<Swiper
					...
					navigation={{
						prevEl: navigationNextRefLive4.current,
						nextEl: navigationPrevRefLive4.current,
					}}
					spaceBetween={10}
					pagination={{
						type: "fraction",
...
export default CasinoLiveOtherSlider;
