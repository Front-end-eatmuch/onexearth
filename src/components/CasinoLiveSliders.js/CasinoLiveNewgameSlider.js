...
function CasinoLiveNewgameSlider(props) {
	// const { casinoData } = useOutletContext();

	const [swiperRef, setSwiperRef] = useState(null);
	const navigationPrevRefLive3 = React.useRef(null);
	const navigationNextRefLive3 = React.useRef(null);
	const navigate = useNavigate();
	const handleClick = () => {
		// 👇️ replace set to true
		// navigate("casino-live/popular", { replace: true });
		window.location.replace("/casino-live/newgame");
	};
	return (
		<div>
			<div className=' relative bg-white w-full overflow-x-hidden py-5 px-5 space-y-5'>
				...
				<Swiper
					...
					navigation={{
						prevEl: navigationNextRefLive3.current,
						nextEl: navigationPrevRefLive3.current,
					}}
					spaceBetween={10}
					pagination={{
						type: "fraction",
...
export default CasinoLiveNewgameSlider;
