...
function CasinoLiveBonusbuySlider(props) {
	// const { casinoData } = useOutletContext();

	const [swiperRef, setSwiperRef] = useState(null);
	const navigationPrevRefLive1 = React.useRef(null);
	const navigationNextRefLive1 = React.useRef(null);
	const navigate = useNavigate();
	const handleClick = () => {
		// 👇️ replace set to true
		// navigate("casino-live/popular", { replace: true });
		window.location.replace("/casino-live/bonusbuy");
	};
	return (
		<div>
			<div className=' relative bg-pink-800 w-full overflow-x-hidden py-5 px-5 space-y-5'>
				...
				<Swiper
					...
					navigation={{
						prevEl: navigationNextRefLive1.current,
						nextEl: navigationPrevRefLive1.current,
					}}
					spaceBetween={10}
					pagination={{
						type: "fraction",
...
export default CasinoLiveBonusbuySlider;
