...
function CasinoLivePopularSlider(props) {
	// const { casinoData } = useOutletContext();

	const [swiperRef, setSwiperRef] = useState(null);
	const navigationPrevRefLive5 = React.useRef(null);
	const navigationNextRefLive5 = React.useRef(null);
	const navigate = useNavigate();
	const handleClick = () => {
		// 👇️ replace set to true
		// navigate("casino-live/popular", { replace: true });
		window.location.replace("/casino-live/popular");
	};
	return (
		<div>
			<div className=' relative bg-white w-full overflow-x-hidden py-5 px-5 space-y-5'>
				...
				<Swiper
					...
					navigation={{
						prevEl: navigationNextRefLive5.current,
						nextEl: navigationPrevRefLive5.current,
					}}
					spaceBetween={10}
					pagination={{
						type: "fraction",
...
export default CasinoLivePopularSlider;
