...
function CasinoPopularSlider(props) {
	// const { casinoData } = useOutletContext();

	const [swiperRef, setSwiperRef] = useState(null);
	const navigationPrevRef5 = React.useRef(null);
	const navigationNextRef5 = React.useRef(null);
	const handleClick = () => {
		// 👇️ replace set to true
		// navigate("casino-live/popular", { replace: true });
		window.location.replace("/casino/popular");
	};
	return (
		<div>
			<div className=' relative bg-fuchsia-800 w-full overflow-x-hidden py-5 px-5 space-y-5'>
				...
				<Swiper
					...
					navigation={{
						prevEl: navigationNextRef5.current,
						nextEl: navigationPrevRef5.current,
					}}
					spaceBetween={10}
					pagination={{
						type: "fraction",
...
export default CasinoPopularSlider;
